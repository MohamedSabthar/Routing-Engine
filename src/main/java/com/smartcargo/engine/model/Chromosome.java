...
public class Chromosome {
	...
	public double calculateTotalDistance() {
		...
		return (int) (this.orders.stream().mapToDouble(x -> {
			...
			if(orderIndex < ordersSize - 1) returnValue = CommonService.caculateHarvasine(x.getLocation(), this.orders.get(orderIndex + 1).getLocation());
//			if(orderIndex < ordersSize - 1) returnValue = x.measureDistance(this.cities.get(cityIndex + 1));
			return returnValue;
		}).sum() + CommonService.caculateHarvasine(this.orders.get(0).getLocation(), this.orders.get(ordersSize - 1).getLocation()));
